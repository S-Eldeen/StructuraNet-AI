...
# ── Platform-specific Dynamips built-in Ethernet port counts ─────────────────
# (adapter 0 = built-in ports; adapter 1+ = expansion slots)
_DYNAMIPS_BUILTIN_ETH: Dict[str, int] = {
    "c7200": 1,   # FastEthernet0/0
    "c3745": 2,   # FastEthernet0/0, FastEthernet0/1
    "c3725": 2,
    "c3660": 2,   # Leopard-2FE: FastEthernet0/0, FastEthernet0/1
    "c3640": 0,   # No built-in Ethernet — must start at adapter 1
    "c3620": 0,
    "c2691": 2,
    "c2600": 1,
    "c1700": 1,
}
_DYNAMIPS_BUILTIN_ETH_DEFAULT = 1
...
class _NodePortState:
    ...
    def next_ethernet(self) -> Tuple[int, int]:
        ...
        if ntype == "dynamips":
            builtin = _DYNAMIPS_BUILTIN_ETH.get(platform, _DYNAMIPS_BUILTIN_ETH_DEFAULT)
        ...
        if ntype == "iou":
            # IOU Ethernet: each adapter provides 4 ports
            result = (self.iou_eth_adapter, self.iou_eth_port)
            self.iou_eth_port += 1
            if self.iou_eth_port >= 4:
                self.iou_eth_port = 0
    ...
    def next_serial(self, all_eth_adapters_used: int) -> Tuple[int, int]:
        ...
        if ntype == "iou":
            ...
            if self.iou_ser_adapter == -1:
                ...
                #   max(IOU_DEFAULT_ETH_ADAPTERS, ceil(eth_links / 4))
                from hw_config import IOU_DEFAULT_ETH_ADAPTERS, IOU_PORTS_PER_ADAPTER
        ...
        self.dyn_ser_port += 1
        if self.dyn_ser_port >= 4:   # PA-4T+ / NM-4T each have 4 ports
            self.dyn_ser_port = 0
